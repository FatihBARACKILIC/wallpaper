...
- **Idle cost is the primary constraint.** No polling timers, no views alive while the menu is closed, no decoded images held in memory. Scheduling is one run-loop timer armed for the next due date — a single wakeup per interval, with `tolerance` so the system can coalesce it.
