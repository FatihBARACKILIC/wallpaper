...
You need macOS 15 Sequoia or later. Either way of installing it ends at the same
place, so pick whichever suits you.

**With Homebrew**

```
brew install --cask FatihBARACKILIC/tap/wallpaper
```

The first time, Homebrew asks you to trust the tap and prints the command that
does it.
...
**Then, the first time you open it**, macOS will say it cannot verify the app.
Click **Done**, open **System Settings → Privacy & Security**, scroll to the
bottom and click **Open Anyway**. You are asked once and never again.

**Why the extra step?** Apple charges developers a yearly fee to have their apps checked and approved. This one has not been through that process, so macOS stops it the first time and asks you to say, once, that you meant it. Nothing is wrong with the download.
